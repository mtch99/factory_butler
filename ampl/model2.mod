# Ensemble
set Produit; # Produit fini fabriqué en usine et vendu aux distributeurs
set Distributeur;  # Ditributeurs
set Usine;  # Usines
set MP; # Matières premières
set Fournisseur; # Ensemble des fournisseurs
...
param qte_mp_par_produit {Produit, MP}; # Quantité de matière première pour fabriquer une unité de chaque produit
param stock_mp_fournisseur {Produit, MP}; # Stock de matière premieère pour chaque fournisseur
...
# Fonction objective
maximize Profit:
    sum {u in Usine, d in Distributeur} ((prix_vente*Ventes[u,d])-(cf_livraison + (cv_transport*distance[u,d])) * Ventes[u,d]) 
    	- sum {d in Distributeur} (Penurie[d] * cf_penurie[d]);
		# Coût MP
		- sum {mp in MP, }     
